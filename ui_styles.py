...
def load_css(theme: str = "dark") -> str:
    ...
    noise_opacity = "0.035" if is_dark else "0.02"
    scanline = f"rgba(0,0,0,0.18)" if is_dark else "rgba(0,0,0,0.04)"
    grid_color = f"rgba(230,57,70,0.04)" if is_dark else "rgba(193,18,31,0.04)"
...
/* Sidebar collapse toggle arrow — keep visible */
...
@keyframes ntd-dot   {{
    0%,80%,100% {{ transform:scale(0.2); opacity:0.15; }}
...
.ntd-ring-outer {{
    ...
    border-radius: 50%;
    animation: ntd-spin 0.9s linear infinite;
...
.ntd-ring-mid {{
    ...
    border-radius: 50%;
    animation: ntd-spinr 0.65s linear infinite;
    box-shadow: 0 0 10px {t.GREEN}44;
}}
.ntd-ring-inner {{
    position: absolute;
    inset: 20px;
    border: 1px solid {t.BORDER};
    border-bottom-color: {t.YELLOW};
    border-radius: 50%;
    animation: ntd-spin 0.45s linear infinite;
}}
.ntd-spin-txt {{
    font-family: 'Share Tech Mono', monospace;
    font-size: 0.72rem;
